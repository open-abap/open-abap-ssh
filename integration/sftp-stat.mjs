  ...
  async zif_oassh_socket$connect() {
    ...
    socket.once("close", hadError => {
      if (hadError || !operationComplete) {
        rejectSocket(new Error("OpenSSH socket closed before SFTP STAT completed"));
      }
    });
  },
...
const stat = statMethod({iv_path: new abap.types.String().set(path)}).then(value => {
  operationComplete = true;
  return value;
});
let timer;
const timeout = new Promise((_, reject) => {
  timer = setTimeout(() => reject(new Error("Timed out waiting for SFTP STAT")), 300000);
});
let attrs;
try {
  attrs = await Promise.race([stat, socketFailure, timeout]);
} finally {
  clearTimeout(timer);
}
await client.close();
